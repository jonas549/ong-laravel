...
/**
 * Las 16 regiones y 346 comunas de Chile, en orden norte a sur.
 */
class RegionCommuneSeeder extends Seeder
{
    public function run(): void
    {
        foreach ($this->data() as $orden => [$region, $comunas]) {
            $r = Region::updateOrCreate(
                ['slug' => Str::slug($region)],
                ['nombre' => $region, 'orden' => $orden + 1],
            );

            foreach ($comunas as $comuna) {
                Commune::updateOrCreate(
                    ['region_id' => $r->id, 'slug' => Str::slug($comuna)],
                    ['nombre' => $comuna],
                );
            }
        }

...
<main style="flex:1;">
    ...
    <div class="rise" style="max-width:820px;margin:0 auto;padding:64px 32px 96px;">
        <div style="text-align:center;margin-bottom:36px;">
            ...
            {{-- El texto sigue el estado REAL. Desde que existe la aprobación
                 automática, una actividad puede salir publicada en el acto, y
                 esta pantalla decía «revisaremos la información» igual: le
                 prometía a la persona una espera que no iba a ocurrir. --}}
            <p style="font-size:17px;line-height:1.65;color:var(--gris);margin:0 auto;max-width:52ch;text-wrap:pretty;">Gracias por abrir sus puertas y ser parte del movimiento que construye el patrimonio vivo de nuestras comunidades.
                @if ($activity->estado === 'publicada')
                    Ya está publicada en el calendario y cualquiera puede verla.
                @else
                    Revisaremos la información y te escribiremos con el resultado.
                @endif
            </p>
        </div>
        ...
        <div style="background:#fff;border:1px solid var(--linea);border-radius:24px;padding:28px;box-shadow:0 18px 40px -32px rgba(0,0,0,.22);margin-bottom:18px;">
            ...
            <div style="display:flex;flex-direction:column;gap:14px;">
                <div style="display:flex;gap:13px;align-items:flex-start;">
                    ...
                    <div style="font-size:15.5px;line-height:1.6;color:var(--gris-700);">
                        @if ($activity->estado === 'publicada')
                            Tu actividad ya aparece en el calendario. Puedes editarla cuando quieras desde tu cuenta.
                        @else
                            Revisaremos tu actividad y te escribiremos con el resultado.
                        @endif
                    </div>
                </div>
                <div style="display:flex;gap:13px;align-items:flex-start;">
                    ...
                    <div style="font-size:15.5px;line-height:1.6;color:var(--gris-700);">
                        @if ($activity->estado === 'publicada')
                            Comparte su enlace y su QR con tus asistentes: así quedan registrados como participantes y pueden evaluar su experiencia.
                        @else
                            Si es aprobada, recibirás la URL y el QR para compartir con tus asistentes: así quedan registrados como participantes y pueden evaluar su experiencia.
                        @endif
                    </div>
                </div>
            </div>

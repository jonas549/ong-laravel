...
<section id="que-es" style="scroll-margin-top:90px;background:linear-gradient(160deg,rgba(92,184,178,.14) 0%,rgba(92,184,178,.07) 100%);">
    <div style="max-width:1180px;margin:0 auto;padding:92px 40px;display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:60px;align-items:center;">
        <div class="reveal">
            <h2 style="font-weight:800;font-size:38px;margin:0 0 22px;letter-spacing:-.01em;">{{ $seccion->texto('titulo_antes', $b) }} <span style="color:var(--naranjo);">{{ $seccion->texto('titulo_destacado', $b) }}</span>{{ $seccion->texto('titulo_despues', $b) }}</h2>
            ...
            <p style="font-size:19px;line-height:1.4;margin:0 0 26px;font-weight:700;color:var(--ink);">{{ $seccion->texto('remate', $b) }}</p>
            <a href="{{ $seccion->enlace('cta_enlace', $b) ?: route('activities.index') }}" class="btn btn-primary">{{ $seccion->texto('cta_texto', $b) }}</a>
        </div>

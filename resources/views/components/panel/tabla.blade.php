...
{{--
    ...
    estado vacío con su colspan, la paginación y —esto es nuevo— la selección
    múltiple con sus acciones.
...
<div {{ $attributes->merge(['class' => 'panel-tabla']) }}
    ...
    @if ($seleccionable)
        <form method="POST" action="{{ $accionesEn }}" x-ref="formulario"
    ...
    @if ($seleccionable)
            {{--
                La barra de acciones aparece al marcar algo y va abajo, pegada a
                la tabla: arriba obligaría a subir después de marcar la última
                fila, que es justo donde está la mano.
            --}}
            <div class="tabla-acciones" x-show="marcadas > 0" x-cloak>
                ...
                <div class="tabla-acciones-botones">
                    @foreach ($acciones as $clave => $accion)
                        <button type="submit" name="accion" value="{{ $clave }}"
